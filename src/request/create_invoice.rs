...
impl<'a> CreateInvoiceRequest<'a> {
    pub async fn send(self) -> anyhow::Result<Invoice> {
        ...
        if let Some(ref unwrapped) = self.retainer_id {
            r = r.push_json(json!({ "retainer_id" : unwrapped }));
        }
        if let Some(ref unwrapped) = self.estimate_id {
            r = r.push_json(json!({ "estimate_id" : unwrapped }));
        }
        ...
        if let Some(ref unwrapped) = self.due_date {
            r = r.push_json(json!({ "due_date" : unwrapped }));
        }
        if let Some(ref unwrapped) = self.payment_term {
            r = r.push_json(json!({ "payment_term" : unwrapped }));
        }
        if let Some(ref unwrapped) = self.line_items_import {
            r = r.push_json(json!({ "line_items_import" : unwrapped }));
        }
    ...
    pub fn retainer_id(mut self, retainer_id: i64) -> Self {
        self.retainer_id = Some(retainer_id);
        self
    }
    pub fn estimate_id(mut self, estimate_id: i64) -> Self {
    ...
    pub fn due_date(mut self, due_date: &str) -> Self {
        self.due_date = Some(due_date.to_owned());
        self
    }
    pub fn payment_term(mut self, payment_term: &str) -> Self {
        self.payment_term = Some(payment_term.to_owned());
        self
    }
